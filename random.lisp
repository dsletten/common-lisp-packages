...
;;;;   Name:               random.lisp
;;;;
;;;;   Started:            Sat Jul 18 14:39:34 2020
;;;;   Modifications:
;;;;
;;;;   Purpose:
;;;;
...
(defpackage :random (:use :common-lisp :test))

(in-package :random)

(let ((random-state *random-state*))
  (defun uniform (&optional (a 1d0) (b 0 b-supplied-p))
    (cond (b-supplied-p
           (assert (< a b))
           (typecase a
             (integer (+ a (uniform (- b a))))
             (float (+ a (* (uniform) (- b a)))) ))
          (t (assert (> a 0))
             (random a random-state))))
  (defun bernoulli (&optional (p 0.5d0))
    (assert (<= 0d0 p 1d0))
    (< (uniform) p))
  (defun set-seed (rs)
